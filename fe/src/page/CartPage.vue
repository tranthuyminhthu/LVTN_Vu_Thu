    ...
    <div class="grid">
      <div class="col-6">
        <div class="flex justify-between mb-4 items-center">
          ...
          <p class="cursor-pointer text-[#2f5acf]" @click="visible = true">
            Chọn từ sổ địa chỉ
          </p>
        </div>
        <form action="" class="mt-2">
          <div class="grid items-center">
              ...
              <div class="flex flex-col gap-2">
                ...
                <InputGroup class="rounded-full h-12">
                  ...
                  <InputText
                    ...
                    :class="[
                      '!rounded-r-full focus:!border-[#cbd5e1]',
                ...
                <p
                  class="text-xs min-h-[16px] mt-1"
                  :class="errors.name ? 'text-red-500' : 'text-transparent'"
                >
                  {{ errors.name || " " }}
                </p>
              </div>
            ...
            <div class="flex flex-col gap-2 col-4">
              ...
              <InputText
                ...
                :class="[
                  '!rounded-full focus:!border-[#cbd5e1] h-12',
              ...
              <p
                class="text-xs min-h-[16px] mt-1"
                :class="errors.phone ? 'text-red-500' : 'text-transparent'"
              >
                {{ errors.phone || " " }}
              </p>
            </div>
          ...
          <div class="flex flex-col gap-1">
            ...
            <p
              class="text-xs min-h-[16px] mt-1"
              :class="errors.email ? 'text-red-500' : 'text-transparent'"
            >
              {{ errors.email || " " }}
            </p>
          </div>
          <div class="flex flex-col gap-1">
            ...
            <InputText
              ...
              :class="[
                '!rounded-full focus:!border-[#cbd5e1] h-12',
                errors.address ? 'border border-red-500' : 'border',
              ]"
              id="address"
            ...
            <p
              class="text-xs min-h-[16px] mt-1"
              :class="errors.address ? 'text-red-500' : 'text-transparent'"
            >
              {{ errors.address || " " }}
            </p>
          </div>
          ...
          <div class="flex gap-2">
            <div class="flex flex-col gap-1 flex-1">
              ...
              <Select
                ...
                :class="[
                  '!rounded-full flex-1',
                  errors.city ? 'border border-red-500' : 'border',
                ]"
                id="city"
              ...
              <p
                class="text-xs min-h-[16px] mt-1"
                :class="errors.city ? 'text-red-500' : 'text-transparent'"
              >
                {{ errors.city || " " }}
              </p>
            </div>
            <div class="flex flex-col gap-1 flex-1" v-if="!useNewProvinceApi">
              ...
              <Select
                ...
                :class="[
                  '!rounded-full flex-1',
                  errors.district ? 'border border-red-500' : 'border',
                ]"
                id="district"
              ...
              <p
                class="text-xs min-h-[16px] mt-1"
                :class="errors.district ? 'text-red-500' : 'text-transparent'"
              >
                {{ errors.district || " " }}
              </p>
            </div>
            <div class="flex flex-col gap-1 flex-1">
              ...
              <Select
                ...
                :class="[
                  '!rounded-full flex-1',
                  errors.ward ? 'border border-red-500' : 'border',
                ]"
                id="ward"
              ...
              <p
                class="text-xs min-h-[16px] mt-1"
                :class="errors.ward ? 'text-red-500' : 'text-transparent'"
              >
                {{ errors.ward || " " }}
              </p>
            </div>
      ...
      <div class="col-6 max-h-screen overflow-y-auto hide-scrollbar">
        <div class="flex items-center justify-between">
          ...
          <Button
            icon="pi pi-trash"
            class="p-button-sm bg-white !text-black !border-black cursor-pointer hover:!bg-black hover:!text-white transition-all duration-300"
        ...
        <!-- <div class="my-2 flex gap-2">
          <input
            type="text"
            class="w-full rounded-full border border-[#e6e6e680] p-2 bg-[#eeeeee]"
            placeholder="Nhập mã giảm giá"
          />
          <button
            class="bg-black text-white px-6 py-1 rounded-full cursor-pointer text-sm w-[30%]"
          >
            Áp dụng Voucher
          </button>
        </div> -->
        ...
        <div class="flex justify-between my-2 border-y py-2 border-[#e6e6e680]">
          ...
          <div class="flex items-center gap-2 flex-col text-right">
            <span class="ml-auto">{{ formatVND(cartTotalPrice) }}</span>
          </div>
        </div>
      </div>
    </div>
      ...
      <div class="flex justify-between">
        ...
        <div class="flex-1 text-right flex items-center justify-end gap-4">
          <div class="flex flex-col">
            <p>
              Thành tiền
              <span class="text-[20px] font-bold text-[#2f5acf]">{{
                formatVND(selectedTotal)
              }}</span>
            </p>
          </div>
          <button
            class="bg-black text-white px-8 py-2 rounded-full cursor-pointer pay-btn"
            @click="handlePayment"
...
const genders = ref([{ name: "Anh" }, { name: "Chị" }, { name: "Anh/Chị" }]);
  ...
  switch (field) {
    ...
    case "district":
      errors.value.district =
        useNewProvinceApi.value || selectedDistrict.value
          ? ""
...
watch([selectedCity, districts], ([city, districtList]) => {
  if (
    autoSelect.value &&
    autoSelect.value.districtName &&
    districtList.length > 0
  ) {
    const district = districtList.find((d) =>
      d.name
        .toLowerCase()
        .includes(autoSelect.value!.districtName!.toLowerCase())
    );
...
watch([selectedDistrict, wards], ([district, wardList]) => {
  if (autoSelect.value && autoSelect.value.wardName && wardList.length > 0) {
    const ward = wardList.find((w) =>
      w.name.toLowerCase().includes(autoSelect.value!.wardName!.toLowerCase())
    );
...
watch(selectedWard, (ward) => {
  if (ward && selectedCity.value && selectedDistrict.value) {
    const cityCode =
      ...
      selectedCity.value.raw?.ProvinceID ||
      selectedCity.value.raw?._id;
    const districtCode =
      selectedDistrict.value.id || selectedDistrict.value.raw?.DistrictID;
...
// Remove the problematic watcher

onMounted(async () => {
  await fetchProvinces();

  // Delay loading cart to avoid recursive updates
  setTimeout(async () => {
    await loadCart();
  }, 100);
});
...
const updateQuantity = async (item: CartItemWithProduct) => {
  try {
  ...
  confirm.require({
    ...
    accept: async () => {
      try {
        ...
        await removeCartItems(itemIdsToDelete);
